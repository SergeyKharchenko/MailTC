...
#define ApplicationName "MailTC 2 v1.0"

AppName    = MailTC 2 v1.0
AppVersion = 1.0
...
;LicenseFile      = sources\1\TestLicense.txt
...
OutputBaseFilename = MailTC 2 v1.0
...
#define inFiles[0] "sources\MailTC 2.mq4"
#define inFiles[1] "sources\MailTC 2.ex4"
#define inFiles[2] "sources\MailTC.dll"
#define inFiles[3] "sources\AE.Net.Mail.dll"

;TODO Output directories and names for above files inside MetaTrader directory
#dim outFiles[filesTotal]
#define outFiles[0] "experts\MailTC 2 v1.0.mq4"
#define outFiles[1] "experts\MailTC 2 v1.0.ex4"
#define outFiles[2] "experts\libraries\MailTC.dll"
#define outFiles[3] "\AE.Net.Mail.dll"
...
Source: "{#inFiles[2]}"; DestDir: "{tmp}"; Flags: dontcopy
Source: "{#inFiles[3]}"; DestDir: "{tmp}"; Flags: dontcopy
...
Source: "sources\dotNetFx45_Full_setup.exe"; DestDir: "{app}\install"; Flags: deleteafterinstall

;TODO (if needed remove comments)
[Run]
Filename: "{app}\install\dotNetFx45_Full_setup.exe"; Description: "Install Microsoft .Net Framework 4.5"; Flags: postinstall waituntilterminated
...
//TODO  Add or remove needed lines here 
    ...
    for i := 0 to GetArrayLength(InstalledTerminals) - 1 do begin
      if (InstalledTerminalsCheckBoxes.Checked[i]) then begin
        ...
        // Copy file #3
        if (not DirExists(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[2]}')))) then begin
          CreateDir(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[2]}')));
        end;
        if FileCopy(ExpandConstant('{tmp}') + '\' + GetFileFromPath(ExpandConstant('{#inFiles[2]}')), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[2]}'), false) then begin
          RegWriteStringValue(HKEY_LOCAL_MACHINE, ExpandConstant(UNINSTALL_SUBKEY + '\{#ApplicationName}'), 'InstallLocation_3_' + IntToStr(i), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[2]}'));
        end;  

        // Copy file #4
        if (not DirExists(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[3]}')))) then begin
          CreateDir(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[3]}')));
        end;
        if FileCopy(ExpandConstant('{tmp}') + '\' + GetFileFromPath(ExpandConstant('{#inFiles[3]}')), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[3]}'), false) then begin
          RegWriteStringValue(HKEY_LOCAL_MACHINE, ExpandConstant(UNINSTALL_SUBKEY + '\{#ApplicationName}'), 'InstallLocation_4_' + IntToStr(i), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[3]}'));
        end;   
